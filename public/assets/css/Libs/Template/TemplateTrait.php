...
use App\Daos\EmbaixadorEventoDao;
use App\Daos\FotografoDao;
...
Trait TemplateTrait
{
    ...
    protected function navigationBottom($menuAtivo = 1, $data = null)
    {
        ...
        $data["PERFILUSUARIO"]["EMBAIXADOR"] = (new EmbaixadorEventoDao)->isEmbaixador();
        $data["PERFILUSUARIO"]["FOTOGRAFO"] = (new FotografoDao)->isFotografo();
    ...
    private function getNavbarColorFromCssFile($themeName) {
        $filePath = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/css/my-theme-color.css';
        ...
        // Remove o prefixo "theme-" se vier junto
        $themeName = str_replace('theme-', '', $themeName);

        $pattern = '/\.theme-' . preg_quote($themeName, '/') . '\s*\{[^}]*--mycolor-navbar:\s*([^;]+);/s';
    ...
    protected function head($data = null)
    {
        ...
        if (!empty($data["THEME_COLOR"])) {
            $result = $this->getNavbarColorFromCssFile($data["THEME_COLOR"]);
            $data["color"] = $result??CONFIG_COLOR["color-primary"];
