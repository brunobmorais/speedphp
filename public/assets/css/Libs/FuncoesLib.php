...
class FuncoesLib
{
    ...
    public function getCurrentDateDatabase($hoursToAdd = 0)
    {
       ...
       public function addMinutosToCurrentDateDatabase($minutesToAdd = CONFIG_SITE['prazo_minutos_pagamento'])
    {
        // Define o fuso horário, caso necessário, se não especificado, usa o fuso horário do sistema
        $timezone = new DateTimeZone('America/Sao_Paulo');
        $date = new DateTime('now', $timezone);
    ...
    function formatDataHoraBanco($date)
    {
        // Verifica se a data está vazia
        if (empty($date)) {
            return '';
        ...
        if ($data) {
            return $data->format('Y-m-d H:i:s'); // Saída: 1981-11-20
        } else {
            return null;
        }
    }
    ...
    {
        return str_replace(array(".", "-", "/", " ", "(", ")", "_", " ", "%0A"), "", $texto);
    }
    ...
    public function pegarArrayDiasDaSemanaSemFeira()
    {
    ...
    public function formatTaxa($valor)
    {
        if ($valor === null || $valor === '') {
            return '';
        }
    ...
    function sanitizeJsonText($text)
    {
        ...
        // Mapeamento de caracteres especiais para suas versões "normais"
        $replacements = [
            'º' => 'o',
            'ª' => 'a',
            '°' => 'o',
        ...
        $sanitized = str_replace(array_keys($replacements), array_values($replacements), $text);

        // Remove outros caracteres de controle e não-ASCII problemáticos
        $sanitized = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', '', $sanitized);

        // Remove caracteres que podem causar problemas em JSON
    ...
    /**
     * Ofusca um nome, mostrando apenas a primeira letra de cada palavra seguida por asteriscos.
     * Exemplo: "Athus Felipe Florentino Santos" se torna "A*** F*** F*** S***".
     *
     * @param string|null $nomeCompleto O nome completo a ser ofuscado.
     * @return string O nome ofuscado.
     */
    public function anonimizarDado(?string $nomeCompleto): string
    {
        if (empty(trim($nomeCompleto))) {
            return "";
        }

        $palavras = preg_split('/\s+/', trim($nomeCompleto));
        $palavrasOfuscadas = [];

        foreach ($palavras as $palavra) {
            $palavrasOfuscadas[] = mb_substr($palavra, 0, 1, 'UTF-8') . '***';
        }

        return implode(' ', $palavrasOfuscadas);
    }


    /**
     * retorna primero e ultimo nome do usuario para facilitar a visualização
     *
     *      * Exemplo: "Athus Felipe Florentino Santos" se torna "Athus Santos".

     * @param string|null $nomeCompleto
     * @return string
     */
    public function primeiroUltimoNome(?string $nomeCompleto): string
    {
    ...
    /**
     * Formata um número de telefone para ser usado em links do WhatsApp.
     * Remove caracteres não numéricos e garante a presença do código de país (55 para o Brasil).
     *
     * @param string|null $telefone O número de telefone a ser formatado.
     * @return string O número de telefone formatado, pronto para URL.
     */
    public function formatarTelefoneWhatsapp(?string $telefone): string
    {
        ...
        if (str_starts_with($numeros, '55') && in_array(strlen($numeros), [12, 13])) {
            return $numeros;
        }

        if (in_array(strlen($numeros), [10, 11])) {
            return '55' . $numeros;
        }
