    ...
    public function assinarDocumento(AssinaturaModel $assinaturaModel, $tentativa = 1): bool
    {
        ...
        try {
            ...
            for ($i = 1; $i <= $pagecount; $i++) {
                ...
                // SE FOR A ÚLTIMA PÁGINA → adiciona assinatura ao final do conteúdo
                if ($i == $pagecount) {
                    // CORREÇÃO: Define a posição Y considerando a altura da página
                    // Coloca no final do conteúdo visível (ajuste conforme necessário)
                    $pdf->SetY($size['height'] - 60); // 60mm do final (ajuste este valor)

                    $pdf->addBlockAssinatura();
                }
            }
